...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    async function fetchAll() {
      ...
      const sessionRes = await fetch(`http://localhost:8000/stripe-session/${sessionId}`)
      const sessionJson = await sessionRes.json()
      setSessionData(sessionJson)

      const chatRes = await fetch(`http://localhost:8000/chat/${sessionId}`)
      const chatJson = await chatRes.json()
      setMessages(chatJson.map((m: any) => ({
  ...
  return (
    <main className="min-h-screen bg-black text-white flex items-center justify-center p-10">
      <div className="max-w-lg w-full">
        ...
        <p className="mb-2">Welcome, <strong>{sessionData?.customer_email}</strong>!</p>
